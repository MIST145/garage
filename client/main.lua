				...
				if dstCheck <= 10.0 then
					...
					if draw then
						...
						if dstCheck <= markerSize - 0.1 then
							...
							ESX.ShowHelpNotification(usable and displayText or "Escolhe o veículo.")
...
function OpenMenuGarage(PointType)
	...
	local elements = {}
		...
		table.insert(elements, {label = 'Carros apreendidos'.." ("..Config.ImpoundPrice.."€)", value = 'return_owned_cars'})
	...
	ESX.TriggerServerCallback('garage:getOutOwnedCars', function(ownedCars)
		...
		for _,v in pairs(ownedCars) do
			if Config.UseVehicleNamesLua == true then
				...
				labelvehicle = '| '..plate..' | '..vehicleName..' | Returnar |'
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_car', {
			...
			ESX.TriggerServerCallback('garage:checkMoneyCars', function(hasEnoughMoney)
				if hasEnoughMoney then
					...
					ESX.ShowNotification('Não tens dinheiro suficiente no banco')
				end
			end)
		end, function(data, menu)
			menu.close()
...
AddEventHandler('garage:hasEnteredMarker', function(zone)
	if zone == 'car_pound_point' then
		...
		CurrentActionMsg  = '~INPUT_PICKUP~ para acessar os apreendidos.'
		CurrentActionData = {}
	end
end)
